...
We are going to package `mmojo-server` with an embedded `.gguf` model file. If you would like to package `mmojo-server` as a stand-alone server without an embedded `.gguf` model file, use this step instead: [6a. Package Mmojo Server (Standalone)](docs/6a-Package-Mmojo-Server-Standalone.md).
