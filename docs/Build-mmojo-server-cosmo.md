...
In this subsetp, we will build `mmojo-server-cosmo`. This build includes an x86_64 and an arm64 build, preliminarily packaged as an Actual Portable Executable (APE). The APE will run on x86 and ARM CPUs, and Windows, Linux, and macOS operating systems.

The Cosmopolitan build does not support profiling or GPUs. It will perform slower than platform specific builds that are CPU only or use available GPU. It supports packaging of support files and .gguf models in the APE, which is a zip archive. Tradeoffs.
